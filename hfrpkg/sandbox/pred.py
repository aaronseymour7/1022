...
def reactant_options(unique_products, wildcard_reactants):
    ...
    for reactant in mapped_reactants:
        ...
        inchi = Chem.MolToInchi(reactant)
        val = get_Hf(inchi)
        if smiles not in unique_smiles:
            if val is not None:
                unique_smiles.add(smiles)
...
def recurHomodesmotic(input_mol, lhs_required=None, rhs_required=None, Substruct=None, Replacement=None):
    ...
    return Balance(x)
def main_cli():
    parser = argparse.ArgumentParser(description="Predictable")
    ...
    if mol is None:
        ...
        return
    result = recurHomodesmotic(input_mol=mol)
    if result is None:
        print('Infeasible')
        return
